      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative overflow-hidden">
          <div className="flex gap-8 animate-scroll-x">
            {/* First set */}
            {clients.map((client) => (
              <Card key={`first-${client.id}`} className="border-border hover:shadow-lg transition-smooth group flex-shrink-0 w-64 bg-background">
            ...
            {clients.map((client) => (
              <Card key={`second-${client.id}`} className="border-border hover:shadow-lg transition-smooth group flex-shrink-0 w-64 bg-background">
                <CardContent className="p-6 flex items-center justify-center h-32">
                  <img 
                    src={client.logo} 
                    alt={client.name}
