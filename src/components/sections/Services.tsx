      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            Comprehensive solutions that bridge the gap between complex sustainability 
            data and meaningful action on the ground.
          </p>
        </div>
            ...
            return (
              <Card key={index} className="border-border hover:shadow-lg transition-smooth group">
                <CardHeader className="text-center pb-4">
                  <div className={`w-20 h-20 mx-auto mb-4 circle-accent flex items-center justify-center ${
                    service.color === 'orange' ? 'gradient-orange' : 
                    service.color === 'green' ? 'gradient-green' : 'gradient-primary'
                  }`}>
                  ...
                  <ul className="space-y-2 text-sm text-left">
                    {service.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start space-x-2">
                        <div className={`w-2 h-2 circle-accent mt-2 ${
                          service.color === 'orange' ? 'bg-diversa-orange' : 
                          service.color === 'green' ? 'bg-diversa-green' : 'bg-primary'
                        }`}></div>
